...
import { readR2Env } from "./r2.ts";
import { fill, messages, parseLocale } from "./i18n.ts";
import { checkoutHref, legal } from "./legal.ts";
import { PADDLE_FAMILY_PRICE, PADDLE_PLUS_PRICE, paddlePriceId } from "./paddle.ts";
...
assert.equal(readR2Env(), undefined);
process.env.R2_ACCOUNT_ID = "acc";
process.env.R2_ACCESS_KEY_ID = "key";
process.env.R2_SECRET_ACCESS_KEY = "sec";
process.env.R2_PUBLIC_URL = "https://media.example.com";
assert.equal(readR2Env()?.bucket, "pet-journal");
assert.equal(readR2Env()?.publicUrl, "https://media.example.com");
...
for (const locale of ["zh", "en", "ko", "ja"] as const) {
  assert.equal(messages[locale].landing.plans.length, 3);
  assert.equal(messages[locale].landing.plans[0].checkout, "");
  assert.equal(messages[locale].landing.plans[1].checkout, "plus");
  assert.equal(messages[locale].landing.plans[2].checkout, "family");
  assert.equal(messages[locale].landing.plans[1].price, "$6");
  assert.match(messages[locale].landing.priceCancel, /\S/);
  assert.equal(messages[locale].landing.priceNote.includes("not live"), false);
}
assert.equal(messages.zh.landing.plans.length, 3);
assert.equal(messages.en.landing.plans[1].price, "$6");
assert.equal(messages.en.landing.plans[2].price, "$12");
assert.equal(messages.en.landing.priceNote.includes("not live"), false);
assert.equal(messages.zh.landing.priceNote.includes("还没接通"), false);
assert.match(messages.en.landing.priceNote, /digital subscription/i);
assert.match(messages.en.landing.priceNote, /not veterinary/i);
assert.equal(checkoutHref(""), "/login");
assert.equal(checkoutHref("plus"), "/login");
delete process.env.NEXT_PUBLIC_PADDLE_CLIENT_TOKEN;
assert.equal(paddlePriceId("plus"), "");
process.env.NEXT_PUBLIC_PADDLE_CLIENT_TOKEN = "live_test";
assert.equal(paddlePriceId("plus"), PADDLE_PLUS_PRICE);
assert.equal(paddlePriceId("family"), PADDLE_FAMILY_PRICE);
assert.equal(paddlePriceId(""), "");
delete process.env.NEXT_PUBLIC_PADDLE_CLIENT_TOKEN;
process.env.NEXT_PUBLIC_CREEM_PLUS_URL = "https://checkout.example/plus";
assert.equal(checkoutHref("plus"), "https://checkout.example/plus");
delete process.env.NEXT_PUBLIC_CREEM_PLUS_URL;
assert.equal(legal.en.privacy.sections.length >= 5, true);
assert.match(legal.en.terms.sections.map((section) => section.body.join(" ")).join("\n"), /digital subscription/i);
assert.match(legal.en.terms.sections.map((section) => section.body.join(" ")).join("\n"), /not veterinary/i);
assert.equal(legal.zh.terms.title.length > 0, true);
assert.equal(legal.ko.privacy.sections.length, legal.en.privacy.sections.length);
assert.equal(legal.ja.terms.sections.length, legal.en.terms.sections.length);
